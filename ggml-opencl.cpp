...
struct scoped_spin_lock {
    ...
    explicit scoped_spin_lock(std::atomic_flag& lock) : lock(lock) {
        while (lock.test_and_set(std::memory_order_acquire)) {
            // TODO remove after confirming this is useless
            throw std::runtime_error("g_cl_pool_lock is unnecessary as cl_kernel is not therad safe");
        }
    }
    ~scoped_spin_lock() {
...
static cl_mem ggml_cl_pool_malloc(size_t size, size_t * actual_size) {
    ...
    cl_mem mem;
    CL_CHECK((mem = clCreateBuffer(context, CL_MEM_READ_WRITE, size, nullptr, &err), err));
    *actual_size = size;
    return mem;
}
...
static cl_int ggml_cl_h2d_tensor_2d(cl_mem dst, size_t offset, const struct ggml_tensor * src, cl_int i3, cl_int i2, cl_event * ev) {
    ...
    for (size_t i1 = 0; i1 < ne1; i1++) {
        ...
        err = clEnqueueWriteBufferRect(queue, dst, CL_FALSE, buffer_origin, host_origin, region, 0, 0, nb0, 0, ((const char *)x) + i1*nb0, 0, nullptr, ev == nullptr ? nullptr : &events[i1]);
        if (err != CL_SUCCESS) {
            if (ev != nullptr) {
...
static void ggml_cl_mul_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const cl_int ne0 = ne00 * ne01 * ne02 * ne03;
    ...
    for (cl_int i03 = 0; i03 < ne03; i03++) {
        for (cl_int i02 = 0; i02 < ne02; i02++) {
            ...
            CL_CHECK(ggml_cl_h2d_tensor_2d(d_X, i0, src0, i03, i02, &h2d_ev));
            ...
            CL_CHECK(clEnqueueReadBuffer(queue, d_D, false, 0, sizeof(float) * ne00*ne01, d, 1, &kernel_ev, nullptr));
...
static void ggml_cl_mul_mat_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const cl_int x_ne = ne01 * ne00;
    const cl_int y_ne = ne11 * ne10;
    const cl_int d_ne = ne11 * ne01;
    ...
    for (cl_int i03 = 0; i03 < ne03; i03++) {
        for (cl_int i02 = 0; i02 < ne02; i02++) {
            ...
            // copy dst to host
            auto * d = (float *) ((char *) dst->data + i02*nb2 + i03*nb3);
            CL_CHECK(clEnqueueReadBuffer(queue, d_D, false, 0, sizeof(float) * d_ne, d, 1, &ev_sgemm, nullptr));
...
static void ggml_cl_mul_mat_f16(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, void * wdata, size_t /* wsize */) {
    ...
    for (cl_int i03 = 0; i03 < ne03; i03++) {
        for (cl_int i02 = 0; i02 < ne02; i02++) {
            ...
            // copy dst to host
            ggml_fp16_t * const tmp = (ggml_fp16_t *) wdata + (ne11 * ne10) * (i03 * ne02 + i02);
            CL_CHECK(clEnqueueReadBuffer(queue, d_D, false, 0, sizeof(ggml_fp16_t) * d_ne, tmp, 1, &ev_sgemm, nullptr));
            CL_CHECK(clReleaseEvent(ev_sgemm));
        }
    }
    CL_CHECK(clFinish(queue));
...
static void ggml_cl_mul_mat_q_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (src0->backend == GGML_BACKEND_CPU) {
        for (cl_int i03 = 0; i03 < ne03; i03++) {
            for (cl_int i02 = 0; i02 < ne02; i02++) {
                CL_CHECK(ggml_cl_h2d_tensor_2d(d_Q, 0, src0, i03, i02, nullptr));
            }
        }
    ...
    for (cl_int i03 = 0; i03 < ne03; i03++) {
        for (cl_int i02 = 0; i02 < ne02; i02++) {
            ...
            if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
                ...
                CL_CHECK(clSetKernelArg(*dmmv, 1, sizeof(float) * specialized_local, nullptr));
                CL_CHECK(clSetKernelArg(*dmmv, 2, sizeof(cl_mem), &d_Y));
                CL_CHECK(clSetKernelArg(*dmmv, 3, sizeof(cl_mem), &d_D));
                CL_CHECK(clSetKernelArg(*dmmv, 4, sizeof(cl_int), &ncols));
                CL_CHECK(clEnqueueNDRangeKernel(queue, *dmmv, 1, nullptr, &global, &specialized_local, 0, nullptr, &ev));
            } else { // general dequantization kernel + CLBlast matrix matrix multiplication
                // convert src0 to fp32 on device
                const size_t global = x_ne / global_denom;
                CL_CHECK(clSetKernelArg(*to_fp32_cl, 0, sizeof(cl_mem), &d_Q));
                CL_CHECK(clSetKernelArg(*to_fp32_cl, 1, sizeof(cl_mem), &d_X));
                CL_CHECK(clEnqueueNDRangeKernel(queue, *to_fp32_cl, 1, nullptr, &global, generalized_local > 0 ? &generalized_local : nullptr, 0, nullptr, &ev));

                // wait for conversion
                CL_CHECK(clEnqueueBarrierWithWaitList(queue, 1, &ev, nullptr));
            ...
            // copy dst to host
            auto * d = (float *) ((char *) dst->data + i02*nb2 + i03*nb3);
            CL_CHECK(clEnqueueReadBuffer(queue, d_D, true, 0, sizeof(float) * d_ne, d, 1, &ev, nullptr));
            CL_CHECK(clReleaseEvent(ev));
        }
    }
    CL_CHECK(clFinish(queue));
